...
<xsl:transform
    ...
    <xsl:template match="row"  mode="eadheader">
        ...
        <eadheader countryencoding="iso3166-1" dateencoding="iso8601" findaidstatus="under_revision" langencoding="iso639-2b" repositoryencoding="iso15511">  
            <eadid>
                <xsl:value-of select="$eadidPrefix"/><xsl:value-of select="replace(unique_identifer,'-','')"/>       
            </eadid>
            <filedesc>
                <titlestmt>
                    <titleproper>
                        <xsl:value-of select="$titleproperPrefix"/><xsl:value-of select="title"/>
                    </titleproper>
    ...
    <xsl:template match="row" mode="archdesc">
        <archdesc level="collection">
            <did>
                ...
                <langmaterial>
                    <language langcode="{$languageCode}"><xsl:value-of select="$language"/></language>
                </langmaterial>
    ...
    <xsl:template name="notes">
        ...
        <xsl:if test="$prefercite != ''">
            <prefercite>
                ...
                <p><xsl:value-of select="$prefercite" disable-output-escaping="yes"/></p>
            </prefercite>
        </xsl:if>
        ...
        <xsl:if test="$arrangement != ''">
            <arrangement>
                ...
                <p><xsl:value-of select="$arrangement" disable-output-escaping="yes"/></p>
            </arrangement>
        </xsl:if>
        <xsl:if test="$acqinfo != ''">
            <acqinfo>
                ...
                <p><xsl:value-of select="$acqinfo" disable-output-escaping="yes"/></p>
            </acqinfo>
        </xsl:if>
        ...
        <xsl:if test="$processinfo != ''">
            <processinfo>
                ...
                <p><xsl:value-of select="$processinfo" disable-output-escaping="yes"/></p>
            </processinfo>
        </xsl:if>
        <xsl:if test="$bioghist != ''">
            <bioghist>
                ...
                <p><xsl:value-of select="$bioghist" disable-output-escaping="yes"/></p>
            </bioghist>
        </xsl:if>
        <xsl:if test="$relatedmaterial != ''">
            <relatedmaterial>
                ...
                <p><xsl:value-of select="$relatedmaterial" disable-output-escaping="yes"/></p>
            </relatedmaterial>
        </xsl:if>
    </xsl:template>
            ...
            <physdesc altrender="whole">
                <xsl:choose>
                    <xsl:when test="collection_level/text()">
                        <extent altrender="materialtype spaceoccupied"><xsl:value-of select="extent_number"/><xsl:text> </xsl:text><xsl:value-of select="extent_type"/></extent>
                    </xsl:when>
                    <xsl:otherwise>
                        <extent altrender="materialtype spaceoccupied"><xsl:value-of select="extent_number"/><xsl:text> </xsl:text><xsl:value-of select="extent_type"/></extent>
                    </xsl:otherwise>
                ...
                <xsl:if test="extent_summary ne ''">
                    <extent altrender="carrier"><xsl:value-of select="extent_summary"/></extent>
                </xsl:if>
            </physdesc>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="container">
        <xsl:if test="instance_type ne '' and container_type_1 ne '' and container_indicator_1 ne ''">
